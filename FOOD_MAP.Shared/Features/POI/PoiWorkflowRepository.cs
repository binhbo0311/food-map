...
public sealed class PoiWorkflowRepository : IPoiWorkflowRepository
{
    ...
    public async Task<IReadOnlyList<PoiHeatmapBucketSummary>> GetOwnerPoiHeatmapSummaryAsync(
        int ownerUserId,
        CancellationToken cancellationToken = default)
    {
        await using var dbContext = await _dbContextFactory.CreateDbContextAsync(cancellationToken);

        var ownerPoiIds = await dbContext.Pois
            .AsNoTracking()
            .Where(x => x.OwnerId == ownerUserId)
            .Select(x => x.Id)
            .ToListAsync(cancellationToken);
        ...
        {
            return Array.Empty<PoiHeatmapBucketSummary>();
        }

        var now = DateTimeOffset.UtcNow;
        var start1Day = now.AddDays(-1);
        var start7Days = now.AddDays(-7);
        var start30Days = now.AddDays(-30);

        var groupedSummaries = await dbContext.UserTours
            .AsNoTracking()
            .Where(x => ownerPoiIds.Contains(x.PoiId) && x.VisitedUtc >= start30Days)
            .GroupBy(x => x.PoiId)
            .Select(g => new PoiHeatmapBucketSummary(
                g.Key,
                g.Count(x => x.VisitedUtc >= start1Day),
                g.Count(x => x.VisitedUtc >= start7Days),
                g.Count()))
            .ToListAsync(cancellationToken);

        var summaryByPoiId = groupedSummaries.ToDictionary(x => x.PoiId, StringComparer.OrdinalIgnoreCase);

        return ownerPoiIds
            .Select(poiId => summaryByPoiId.TryGetValue(poiId, out var summary)
                ? summary
                : new PoiHeatmapBucketSummary(poiId, 0, 0, 0))
            .OrderByDescending(x => x.TourCount30Days)
            .ThenByDescending(x => x.TourCount7Days)
            .ThenByDescending(x => x.TourCount1Day)
            .ThenBy(x => x.PoiId)
            .ToList();
    }
