...
-- 6) FoodItems table for POI type = food
...
-- 7) Optional test data for Phase 2 flows
...
SELECT
    ...
    'QR-ST-903'
FROM "Users" a
WHERE a."UserName" = 'phase2_admin'
  AND NOT EXISTS (SELECT 1 FROM "POIs" p WHERE p."Id" = 'ST-903');
...
SELECT
    'ST-903', l."Id",
    CASE WHEN l."LanguageCode" = 'vi' THEN 'Điểm Lưu Trú Test' ELSE 'Test Stay Location' END,
    CASE WHEN l."LanguageCode" = 'vi'
         THEN 'POI lưu trú đã duyệt để test phân loại stayin.'
         ELSE 'Approved stay-in POI for category testing.' END,
    'https://picsum.photos/seed/st903/' || l."LanguageCode" || '/1200/800',
    'audio/' || l."LanguageCode" || '/st-903.mp3',
    CASE WHEN l."LanguageCode" = 'vi'
         THEN 'Bạn đang ở điểm lưu trú thử nghiệm.'
         ELSE 'You are near the test stay location.' END
FROM "Languages" l
WHERE l."LanguageCode" IN ('vi', 'en')
ON CONFLICT ("PoiId", "LanguageId") DO UPDATE
SET
...
INSERT INTO "UserTours" ("UserId", "PoiId", "LanguageCode", "TriggerType", "VisitedUtc")
SELECT u."Id", 'ST-903', 'en', 'QrScan', NOW() - INTERVAL '1 day'
FROM "Users" u
WHERE u."UserName" = 'phase2_user'
  AND NOT EXISTS (
      SELECT 1 FROM "UserTours" t
